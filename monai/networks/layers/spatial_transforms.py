...
class AffineTransform(nn.Module):
    ...
    def __init__(self,
        ...
        if theta_shape in ((2, 3), (3, 4)):  # needs padding to dxd
            ...
            self.theta = torch.cat([self.theta, pad_affine], dim=1)
        if tuple(self.theta.shape[1:]) not in ((3, 3), (4, 4)):
            raise ValueError('affine must be Nx3x3 or Nx4x4')
    ...
    def forward(self, src):
        ...
        if theta.shape[0] != src_size[0]:
            raise ValueError('batch dimension of affine and image does not match, got affine: {} and image: {}.'.format(
                theta.shape[0], src_size[0]))

        grid = nn.functional.affine_grid(theta=theta[:, :sr], size=dst_size, align_corners=self.align_corners)
        dst = nn.functional.grid_sample(input=src.contiguous(),
                                        grid=grid,
                                        mode=self.mode,
